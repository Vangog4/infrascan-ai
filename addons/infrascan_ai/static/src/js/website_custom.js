...
(function () {
  ...
  function initCursorGlow() {
    ...
    Object.assign(orb.style, {
      ...
      borderRadius: '50%', pointerEvents: 'none', zIndex: '8888',
      background: 'radial-gradient(circle, rgba(255,107,53,0.07) 0%, rgba(76,201,240,0.03) 40%, transparent 68%)',
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        (function update(now) {
          ...
          el.textContent = Math.round((1 - Math.pow(2, -10 * t)) * target).toLocaleString('ru-RU');
    ...
    qsa('.btn-primary').forEach((btn) => {
      btn.addEventListener('click', function (e) {
        ...
        Object.assign(dot.style, {
          ...
          transform: 'scale(0)', animation: 'isRipple 0.55s ease forwards',
          pointerEvents: 'none',
        });
        if (!document.getElementById('is-ripple-kf')) {
          const s = document.createElement('style');
          s.id = 'is-ripple-kf';
          s.textContent = '@keyframes isRipple{to{transform:scale(1);opacity:0}}';
  ...
  function initHeroFix() {
    ...
    btns.forEach((btn, i) => {
      const strip = ['color', 'background-color', 'border-color', 'border-width', 'border-style'];
      if (i === 0) {
  ...
  function initNavAndSections() {
    ...
    qsa('header#top .nav-link, header#top a.navbar-item').forEach((a) => {
      ...
      for (const rule of navLinkMap) {
        if (rule.texts.some(t => text.includes(t))) {
          a.href = rule.href;
          if (rule.target) { a.target = rule.target; a.rel = 'noopener noreferrer'; }
    ...
    document.querySelectorAll('a[href="#"]').forEach((a) => {
      ...
      if (text.includes('запис') || text.includes('заказ') ||
          text.includes('начат') || text.includes('попроб') || text === '') {
        a.href = BOT_URL;
        a.target = '_blank';
        a.rel = 'noopener noreferrer';
      }
    ...
    qsa('.s_key_benefits a.btn, .s_pricing_table a.btn, .s_call_to_action a.btn').forEach((btn) => {
      const text = btn.textContent.trim();
      if (text.includes('Заказ') || text.includes('Запис') || text.includes('Получи')) {
    ...
    qsa('a.btn').forEach((btn) => {
      const text = btn.textContent.trim();
      if ((text.includes('аудит') || text.includes('инженер')) &&
          (!btn.href || btn.href.endsWith('/#') || btn.getAttribute('href') === '#')) {
        btn.href = FORM_URL;
        btn.removeAttribute('target');
      }
    });
  ...
  function initImageAlts() {
    const altMap = [
      [/\/web\/image\/website\/\d+\/logo/, 'ИнфраСкан — тепловизионная диагностика'],
    ...
    document.querySelectorAll('img').forEach((img) => {
      ...
      if (!img.alt || img.alt === 'My Website') {
        for (const [pat, alt] of altMap) {
          if (pat.test(src)) { img.alt = alt; break; }
        }
      }
    });
  }
